...
function AddSheetData(WorksheetName,input){
  ...
  Spreadsheet.load({
    ...
    oauth2: {
      ...
      "refresh_token": "1//0evIcPldQOPcACgYIARAAGA4SNwF-L9Ir2OXgclUAqto4L7IUhAeJSnIPCo6CAaICs7IL2w5zMNBFHQRrHryH5r7G5NJgkplDIR4"
    }},function run(err,spreadsheet){
      if(err) throw err;
      spreadsheet.receive(function(err,rows,info){
        if(err) throw err;
        NextRows = info.nextRow;

        //transfer input data for term 
        for(i = 1; i<= Object.keys(input).length; i++)
          term[i] = input[input_keys[i-1]];
        input_obj[NextRows] = term ;  //form object

        //add data into sheet(rows type)
